...
// Monday-based offset for day 1 of the month
function firstDayOffset(year, month) {
  const day = new Date(year, month, 1).getDay(); // 0=Sun
  return (day + 6) % 7;
}

export default function CalendarView({ items, todos, onTodosChange }) {
  ...
  // Index todos and payment items by date string
  const todosByDate = {};
          ...
          {cells.map((day, i) => {
            ...
            return (
              <div key={key} className={`cal-cell${isToday ? ' cal-cell--today' : ''}`}>
                ...
                {dayTodos.map((t) => (
                  <div key={t.id} className="cal-todo-chip">
                    {t.description}
                  </div>
                ))}
                {dayPayments.map((p) => (
                  <div key={p.id} className={`cal-payment-chip${p.purchased ? ' cal-payment-chip--done' : ''}`}>
